...
class LoginPage(Famcy.FamcyPage):
	...
	def login_submit(self, submission_obj, info_list):
		if Famcy.FamcyLoginManager.login(info_list):
			_url = request.args["next"].replace(" ", "/") if "next" in request.args.keys() else "/"+Famcy.FManager["ConsoleConfig"]['main_page']
			response = Famcy.RedirectPage(redirect_url=_url)
		else:
			response = Famcy.UpdateAlert(alert_type="alert-warning", alert_message="登入驗證失敗")
		return response

LoginPage.register("/iam/login", Famcy.LoginStyle(), event_source_flag=True)
...
class LogoutPage(Famcy.FamcyPage):
	...
	def render(cls, init_cls=None):
		...
		return redirect(url_for("PageBlueprint.famcy_route_func_name_"+Famcy.FManager["ConsoleConfig"]['login_url'].replace("/", "_")))

LogoutPage.register("/logout", Famcy.LoginStyle(), event_source_flag=True)

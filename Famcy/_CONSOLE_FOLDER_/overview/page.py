...
class OverviewPage(Famcy.FamcyPage):
    ...
    def update_video_frame(self, submission_obj, info_list):
        res = requests.get(Famcy.FManager["ConsoleConfig"]["main_url"]+"/ipcam" + '?' + urllib.parse.urlencode({"address": "rtsp://wowzaec2demo.streamlock.net/vod/mp4:BigBuckBunny_115k.mov", "timeout": 15, "datetime": time.time()}))
        print("res", res)
        self.card_4.layout.content[0][0].update({
            "img_name": ["data:image/jpeg;charset=utf-8;base64,"+res.text]

...
class inputBtn(Famcy.FamcyInputBlock):
    ...
    def render_inner(self):
        ...
        input_html = '<div id="' + self.id + '" class="inputBtn"><label for="' + self.id + "_inputBtn" + '">' + self.value["title"] + '</label><p>' + self.value["desc"] + '</p><input type="' + self.value["input_type"] + '" id="' + self.id + "_inputBtn" + '" name="' + self.name + '" placeholder="' + self.value["placeholder"] + '"' + self.extra_keyup + addition_text + self.mandatory + '><input type="submit" name="send" value="' + self.value["button_name"] + '"></div>' + self.extra_script
        return input_html
